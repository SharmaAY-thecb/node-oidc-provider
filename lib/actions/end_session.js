...
module.exports = function endSessionAction(provider) {
  ...
  const loadClient = function* loadClient(clientId) {
    try {
      debug(`Loading client with clientId: ${clientId}`);
      ...
      debug(`Client loaded successfully: ${clientId}`);
      return client;
    } catch (err) {
      debug('Error loading client:', err.message);
      throw err;
    }
  };
    ...
    get: compose([
      ...
      function* endSessionChecks(next) {
        try {
          ...
          debug('Starting endSessionChecks', params);
          ...
          if (params.id_token_hint) {
            ...
            if (params.post_logout_redirect_uri) {
              try {
                ...
                debug('Error validating post_logout_redirect_uri:', err.message);
                throw err;
              }
            }
          ...
          debug('Error in endSessionChecks:', err.message);
          throw err;
        }
      },
      ...
      function* renderLogout(next) {
        try {
          ...
          const formhtml = `<form id="op.logoutForm" method="post" action="${this.oidc.urlFor(
            'end_session'
          ...
          debug('Error rendering logout page:', err.message);
          throw err;
        }
      },
    ]),
    ...
    post: compose([
      ...
      function* checkLogoutToken(next) {
        try {
          ...
          this.assert(
            this.oidc.session.logout,
            new errors.InvalidRequestError('could not find logout details')
          );
          this.assert(
            this.oidc.session.logout.secret === this.oidc.params.xsrf,
            new errors.InvalidRequestError('xsrf token invalid')
          );
          debug('Logout token validated successfully');
          yield next;
        } catch (err) {
          debug('Error checking logout token:', err.message);
          throw err;
        }
      },
      ...
      function* endSession(next) {
        try {
          ...
          if (this.oidc.params.logout) {
            if (instance(provider).configuration('features.backchannelLogout')) {
              try {
                ...
                const logouts = clientIds.map((visitedClientId) =>
                  Client.find(visitedClientId).then((visitedClient) => {
                    if (visitedClient && visitedClient.backchannelLogoutUri) {
                      return visitedClient.backchannelLogout(
                        this.oidc.session.accountId(),
          ...
          const uri = redirectUri(
            ...
            params.state != null ? { state: params.state } : undefined
          ...
          debug('Error ending session:', err.message);
          throw err;
        }
      },
    ]),
