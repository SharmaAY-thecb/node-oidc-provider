...
module.exports = function endSessionAction(provider) {
  ...
  const loadClient = function* loadClient(clientId) {
    console.log(`Loading client with clientId: ${clientId}`); // Log client loading
    ...
    console.log(`Client loaded successfully: ${clientId}`); // Log successful client load
  ...
  return {
    get: compose([
      ...
      function* endSessionChecks(next) {
        ...
        console.log('Starting endSessionChecks', params); // Log incoming parameters
        ...
        if (params.id_token_hint) {
          ...
          const clientId = (() => {
            try {
              ...
              console.error('Error decoding id_token_hint:', err.message);
          ...
          try {
            ...
            console.log('id_token_hint validated successfully');
          } catch (err) {
            console.error('Error validating id_token_hint:', err.message);
          ...
          if (params.post_logout_redirect_uri) {
            ...
            console.log('post_logout_redirect_uri validated successfully');
          }
      ...
      function* renderLogout(next) {
        ...
        console.log('Logout page rendered successfully');
        yield next;
      },
    ]),
    ...
    post: compose([
      ...
      function* checkLogoutToken(next) {
        console.log('Checking logout token');
        ...
        console.log('Logout token validated successfully');
        yield next;
      },
      ...
      function* endSession(next) {
        ...
        if (this.oidc.params.logout) {
          if (instance(provider).configuration('features.backchannelLogout')) {
            try {
              ...
              yield logouts;
              console.log('Backchannel logout completed');
            } catch (err) {
              console.error('Error during backchannel logout:', err.message);
            }
          }
        ...
        console.log('Redirecting to post logout URI:', uri);
        this.redirect(uri);
